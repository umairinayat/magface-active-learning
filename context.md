...
The current repo uses a unified root `config.yaml` and these primary scripts:

- `img_embedding.py` — single-image embedding + pair fine-tune API
- `run_img_embedding.py` — CLI for embedding and fine-tuning
- `eval_function_pairs.py` — evaluate labeled pairs using config + cosine similarity
- `finetune_and_update_config.py` — update config with a new checkpoint path
- `finetune_on_feedback.py` / `inference_with_embedding.py` — legacy utilities (optional)
...
# 3. Compute Cosine-Margin Contrastive Loss
similarity = torch.sum(emb1 * emb2, dim=1)  # cosine similarity for normalized embeddings
loss_same = label * (1 - similarity) ** 2
loss_diff = (1 - label) * torch.clamp(similarity - margin, min=0.0) ** 2
...
**Evidence from MagFace_repo inference (gen_feat.py):**
```python
transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize(mean=[0., 0., 0.], std=[1., 1., 1.]),
])
# This keeps values in [0, 1] (no mean/std shift).
```
...
The core of our fine-tuning approach is **Cosine-Margin Contrastive Loss**, which learns embeddings by:
- **Pulling same-person pairs closer** (maximize cosine similarity)
- **Pushing different-person pairs apart** (push similarity below a margin)

#### Mathematical Formulation

```python
similarity = cos(emb1, emb2)  # embeddings are L2-normalized

# For same person (label = 1):
loss_same = (1 - similarity)²

# For different person (label = 0):
loss_diff = max(0, similarity - margin)²
...
**Same person pairs:**
- If similarity is low → high loss → model learns to increase similarity
- If similarity is high → low loss → model is already good

**Different person pairs:**
- If similarity > margin → high loss → model learns to decrease similarity
- If similarity ≤ margin → zero loss → pairs are already separated

**Margin parameter (default: 0.4):**
- Defines maximum allowed similarity for different-person pairs
- Too high: negatives get no push
- Too low: model may over-separate
...
4. **MagFace_repo Inference Pipeline:** Reference implementation that clarified preprocessing
